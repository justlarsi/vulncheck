...
def get_system_info():
    return {
        ...
        "Network Interfaces": ", ".join(psutil.net_if_addrs().keys())
    }

def check_antivirus():
    antivirus_processes = ["antivirus", "avast", "avg", "kaspersky", "norton", "mcafee"]
...
def check_firewall():
    firewall_active = os.system("netsh advfirewall show allprofiles state") == 0
...
def check_unauthorized_startup_programs():
    ...
    return "No suspicious startup programs detected.✅"

def check_disk_encryption():
    encryption_status = os.system("manage-bde -status") == 0
    return "Disk is encrypted.✅" if encryption_status else "Warning: Disk is not encrypted.⛔"
...
def check_suspicious_processes():
    ...
    if suspicious_processes:
        return f"Warning: Suspicious processes running ⛔: {', '.join(suspicious_processes)}"
...
def check_running_services():
    ...
    unnecessary_services = [service.display_name() for service in services if service.status() == 'running' and "vulnerable_service" in service.display_name().lower()]
    if unnecessary_services:
        return f"Warning: Unnecessary or vulnerable services running ⛔: {', '.join(unnecessary_services)}"
    return "No unnecessary services running.✅"

def check_system_integrity():
    integrity_check = os.system("sfc /scannow") == 0
...
class ScanDialog(QDialog):
 ...
 border-radius: 8px;
    ...
    def scan_system(self):
        ...
        startup_check = check_unauthorized_startup_programs()
        ...
        suspicious_processes_status = check_suspicious_processes()
        self.progress.setValue(60)

        # Check for unauthorized user accounts
        unauthorized_user_status = check_unauthorized_users()
        self.progress.setValue(70)

        # Check for running services that might be risky
        running_services_status = check_running_services()
        self.progress.setValue(80)

        # Check system integrity
        integrity_status = check_system_integrity()
        ...
        system_info_label = QLabel(
            ...
            f"\n"
            f"Disk Encryption: {encryption_status}\n"
            f"\n"
            f"Suspicious Processes: {suspicious_processes_status}\n"
            f"\n"
            f"Suspicious Startup Programs: {startup_check}\n"
            f"\n"
            f"Running Services: {running_services_status}\n"
            f"\n"
            f"Security Updates: {update_status}\n"
            f"\n"
            ...
            f"Network Interfaces: {system_info['Network Interfaces']}\n"
        )
    ...
    def scan_ports(self):
        self.clear_scroll_area()
        self.loading_label.setText("Scanning ports...Please Wait this might take time....")
        self.progress.setValue(40)
